...
`deferred_tools_delta`(v2.1.69에서 도입된 도구 지연 로딩 기능) 때문에 `--resume`(이전 대화를 이어서 하는 옵션)을 사용하면 첫 번째 메시지의 구조가 서버에 저장된 캐시 버전과 달라집니다. 그래서 캐시가 완전히 무효화(캐시 미스)됩니다. 50만 토큰짜리 대화에서 resume을 한 번 하면 약 $0.15 정도의 할당량이 낭비되는 셈입니다.
